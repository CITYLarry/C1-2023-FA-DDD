...
import co.com.sofka.cityhotel.booking.business.booking.HireServiceUseCase;
...
public class HireServiceUseCaseTest {

    @Mock
    private EventsRepository eventsRepository;

    @InjectMocks
    private HireServiceUseCase hireServiceUseCase;

    @DisplayName("Hire service test")
    @Test
    void testHireService() {
        ...
        HireServiceCommand hireServiceCommand = new HireServiceCommand(
                ServiceType.ServiceList.WIFI,
                "testBookingId"
        );

        Mockito.when(eventsRepository.findAggregateRootId(hireServiceCommand.getBookingId()))
        ...
        Mockito.when(eventsRepository.saveEvent(ArgumentMatchers.any(HiredService.class)))
                .thenAnswer(invocationOnMock -> invocationOnMock.getArgument(0));

        List<DomainEvent> result = hireServiceUseCase.apply(hireServiceCommand);

        Assertions.assertEquals(1, result.size());

        HiredService service = (HiredService) result.get(0);

        Assertions.assertEquals(ServiceType.ServiceList.WIFI, service.getServiceType().value());
    }
}
