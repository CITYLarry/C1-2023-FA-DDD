...
import co.com.sofka.cityhotel.booking.business.client.RemoveGuestUseCase;
...
public class RemoveGuestUseCaseTest {

    @Mock
    private EventsRepository eventsRepository;

    @InjectMocks
    private RemoveGuestUseCase removeGuestUseCase;

    @DisplayName("Remove guest test")
    @Test
    void testRemoveGuest() {
        ...
        RemoveGuestCommand removeGuestCommand = new RemoveGuestCommand(
                "testGuestId",
                "testClientId"
        );

        Mockito.when(eventsRepository.findAggregateRootId(removeGuestCommand.getClientId()))
        ...
        Mockito.when(eventsRepository.saveEvent(ArgumentMatchers.any(RemovedGuest.class)))
                .thenAnswer(invocationOnMock -> invocationOnMock.getArgument(0));

        List<DomainEvent> result = removeGuestUseCase.apply(removeGuestCommand);

        Assertions.assertEquals(1, result.size());

        RemovedGuest guest = (RemovedGuest) result.get(0);

        Assertions.assertEquals("testGuestId", guest.getGuestId().value());
    }
}
